...
public class Enemy : MonoBehaviour {
	...
	void Awake() {
        ...
        if (SpawnBullets == null)
        { // if AudioSource is missing
            Debug.LogWarning("Spawner missing from this gameobject");
        }
	...
	void Update () {
		if (!isStunned)
		{
			if (Time.time >= _moveTime) {
				EnemyMovement();
                RightCheck.GetComponent<BoxCollider2D>().enabled = true;
	...
	public void Stunned()
	{
		...
		{
			isStunned = true;
            RightCheck.GetComponent<BoxCollider2D>().enabled = false;
            if (this.gameObject.tag == "Ogre")
            {
                SpawnBullets.GetComponent<BulletSpawner>().CancelInvoke("SpawnNext");
            }
